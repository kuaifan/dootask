...
const codeOffset = 33;  // 代码版本号偏移量

const envFilePath = path.resolve(process.cwd(), ".env");
const defaultAiSystemPrompt = "你是一位软件发布日志编辑专家。请产出 Markdown 更新日志，面向普通用户，以通俗友好的简体中文描述更新带来的直接好处，避免技术术语。所有章节标题必须以 `### ` 开头并保持英文 Title Case（例如 `### Features`、`### Bug Fixes`、`### Performance`、`### Documentation` 等）。";
...
function buildDefaultUserPrompt(version, changelogSection) {
    return [
        ...
        "5. 若某个小节没有内容，请省略整段小节（包括标题）。",
        "6. 输出仅为 Markdown changelog 内容，不加其他解释。",
        "",
        "以下是原始日志：",
        "```markdown",
        changelogSection,
...
async function enhanceWithAI(version, changelogSection) {
    ...
    const parsedTemperature = Number.parseFloat(process.env.CHANGELOG_AI_TEMPERATURE ?? "");
    const temperature = Number.isFinite(parsedTemperature) ? parsedTemperature : 0.3;
